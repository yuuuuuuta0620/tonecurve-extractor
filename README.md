...
マスターカーブは自由度が少なすぎて per-frame の作業を吸収できません。
つまり**過学習が起きないので、1 ペアでも 6 ペアでもほぼ同じ結果**になります。
未知の写真に適用したときの実測値（6 試行の中央値と最良〜最悪）：
...
`tone` はペアを増やしても中央値がほとんど動きません（6.42 → 6.06）。**1 ペアで既に頭打ち**です。
`curves` は 1 ペアだと 1.28〜4.39 と 3 倍以上振れますが、`tone` は 5.97〜7.55 に収まります。
そのため `tone` では 1 ペアの警告を出しません（露出の注意だけ残ります）。

平均的な精度と引き換えに**予測可能性**を買うモード、と考えてください。
サンプルが 1 組しか無い場合、あるいは色は自分で作ると決めている場合は、これが正解です。
